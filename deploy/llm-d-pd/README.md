...
- Install the Gateway API Inference Extension CRDs and create the namespace:
  ...
  kubectl apply -f [https://github.com/kubernetes-sigs/gateway-api-inference-extension/releases/download/v1.5.0/v1-manifests.yaml](https://github.com/kubernetes-sigs/gateway-api-inference-extension/releases/download/v1.5.0/v1-manifests.yaml)
...
kubectl run curl-debug --rm -it --image=cfmanteiga/alpine-bash-curl-jq --env="IP=$IP" -- \
  ...
  curl -sS -X POST [http://${IP}/v1/completions](http://${IP}/v1/completions) -H 'Content-Type: application/json' -d '{
...
kubectl -n llm-d-pd-mock logs -l [llm-d.ai/role=prefill](http://llm-d.ai/role=prefill) -c modelserver | grep "advertised KV"

kubectl -n llm-d-pd-mock logs -l [llm-d.ai/role=decode](http://llm-d.ai/role=decode)  -c modelserver | grep "pulled remote KV"
